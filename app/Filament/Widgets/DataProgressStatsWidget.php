...
class DataProgressStatsWidget extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        $studyGroups = StudyGroup::where('academic_year_id', $activeYear->id)
        ...
        $currentGrades = Grade::where('academic_year_id', $activeYear->id)->count();
        ...
        $currentRapor = StudentRapor::where('academic_year_id', $activeYear->id)->count();
        $raporPercent = $expectedRapor > 0 ? round(($currentRapor / $expectedRapor) * 100, 1) : 0;
        if ($raporPercent > 100) $raporPercent = 100;

        // 3. Data Siswa
        $totalStudents = Student::count();
        $studentMissingNisn = Student::whereNull('nisn')->orWhere('nisn', '')->count();
