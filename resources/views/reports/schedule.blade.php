...
<body class="bg-gray-100 p-0">
    ...
    <div class="paper-container {{ !$loop->last ? 'page-break mb-8' : '' }}">
        ...
        <div class="mt-4 flex justify-between px-8 text-[8px]">
            ...
            <div class="text-center w-32">
                <p>{{ ucwords(strtolower($school->village ?? $school->city ?? 'Banyuwangi')) }}, {{ $rombel && $rombel->academicYear && $rombel->academicYear->schedule_date ? \Carbon\Carbon::parse($rombel->academicYear->schedule_date)->translatedFormat('d F Y') : now()->translatedFormat('d F Y') }}</p>
